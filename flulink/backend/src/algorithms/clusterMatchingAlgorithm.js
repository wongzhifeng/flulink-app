    ...
    try {
      ...
      for (const candidate of candidatePool) {
        ...
        for (const coreUser of coreUsers) {
          const resonance = await this.resonanceCalculator.calculateResonance(coreUser, candidate);
      ...
      return candidatesWithResonance;
    } catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < members.length; i++) {
        for (let j = i + 1; j < members.length; j++) {
          const resonance = await this.resonanceCalculator.calculateResonance(members[i], members[j]);
